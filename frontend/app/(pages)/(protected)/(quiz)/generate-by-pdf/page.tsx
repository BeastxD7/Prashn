...
const GenerateQuizByPdf = () => {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      if (quizId) {
        ...
        console.error('Quiz ID not found in response data', data)
        toast.error('Generated quiz ID not found.')
      }

      // If backend returned credits charged, update local cached credits
      if (typeof (data as any).creditsCharged === 'number') {
        setUserCredits((prev) => {
          if (prev === null) return prev
          return Math.max(0, prev - (data as any).creditsCharged)
        })
      }
    } catch (error: any) {
      const message =
  ...
  return (
    <div className="min-h-screen w-full bg-linear-to-br from-indigo-50 via-blue-50/30 to-sky-50/20 dark:from-gray-950 dark:via-indigo-950/20 dark:to-blue-950/10 relative">
      {/* Decorative blur orbs */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-20 right-1/3 w-96 h-96 bg-indigo-400/20 dark:bg-indigo-500/5 rounded-full blur-3xl" />
